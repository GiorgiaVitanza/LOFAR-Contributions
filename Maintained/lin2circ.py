...
def main(options):
            ...
            t.addcols(
                pt.maketabdesc(
                    pt.makearrcoldesc(
                        ...
                        0.0,
                        valuetype="complex",
                        shape=numpy.array(t.getcell(column, 0)).shape,
                    )
                ),
                coldmi,
            )
    ...
    if options.poltable:
        ...
        if options.back:
            tp.putcol(
                "CORR_TYPE", numpy.array([[9, 10, 11, 12]], dtype=numpy.int32)
            )  # FROM CIRC-->LIN
        else:
            tp.putcol(
                "CORR_TYPE", numpy.array([[5, 6, 7, 8]], dtype=numpy.int32)
            )  # FROM LIN-->CIRC
...
opt.add_option(
    "-o", "--outcol", help="Output column [default DATA_CIRC]", default="DATA_CIRC"
)
opt.add_option(
    "-p",
    "--poltable",
    help="Update POLARIZATION table? [default False]",
    default=False,
    action="store_true",
)
opt.add_option(
    "-b",
    "--back",
    help="Go back to linear polarization [default False]",
    default=False,
    action="store_true",
)
opt.add_option(
    "-l",
    "--lincol",
    help="Output linear polarization column, if the -b switch is used [default DATA_LIN]; we want to keep the original DATA column",
    default="DATA_LIN",
)
